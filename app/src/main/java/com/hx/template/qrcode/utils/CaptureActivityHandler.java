...
import com.hx.template.qrcode.decode.zbar.ZbarDecodeThread;
...
public class CaptureActivityHandler extends Handler {
     ...
     * @param activity
     * @param cameraManager
     * @param decodeMode    decodeMode大于0为Zxing解码 否则为Zbar解码
     */
    public CaptureActivityHandler(CaptureActivity activity, CameraManager cameraManager, int decodeMode) {
        this.activity = activity;
        if (decodeMode > 0) {
            decodeThread = new ZxingDecodeThread(activity, decodeMode);
        } else {
            decodeThread = new ZbarDecodeThread(activity);
        }
        decodeThread.start();
        state = State.SUCCESS;
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            ...
            case R.id.decode_succeeded:
                ...
                activity.handleDecode( message.obj, bundle);
                break;
